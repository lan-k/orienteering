...
events <- c("SASprintChampsOY2023.csv","LadyAliceOY2023.csv",
            "WilpenaSpursOY2023.csv",
            "ManawarraOY2023.csv",
            "2023-06-24_NightChamps.csv",
            "KeynesGapOY2023.csv",
            "20230820_MerrideeOY_Splits.csv",
            "CrookedStraightOY2023.csv") 

wallaringa_events <- c(5,7)  # different format

fix_event = 4 #new S classes created for Mannawarra instead of long champs

fix_yr = 7 # #wrong year of births
load(file="age_groups.rds")

S_offered <- c(0,1,1,0,1,0,1,0)  #short
B_offered <- c(0,1,1,1,1,1,1,1)  #B class
E_offered <- c(0,0,0,0,0,0,0,0)   #elite





MTBO = 99 #MTBO cancelled 2023, use 99
night_champs = 5 
...
#officials
off1 <- c("Ben Cazzolato","Toby Cazzolato","Phil Hazell")
off2 <- c("Phil Hazell","Joanna George","Vanessa Round")  
off3 <- c("Jack Marschall","Nigel Dobson",NA_character_)
off4 <- c("Andrew McComb",NA_character_,NA_character_)
off5 <- c("Lexie Ashforth","Rachael Upton",NA_character_)
off6 <- c("Steve Cooper",NA_character_,NA_character_)
off7 <- c("Jenny Casanova",NA_character_,NA_character_)
off8 <- c("Peter Mayer","Robert Tucker","Robin Uppill")
...
colnames(officials) <- c("Name1", "Name2","Name3")
...
for (i in seq(length(events))) { 
  ...
  if (i == fix_yr) {
   ...
   left_join(age_group) 
    ...
    r = r %>%
      mutate(#YB=year(parse_date_time(YB,"y")),
  ...
  r <- r %>%
    ...
    mutate(Name=paste(First.name,Surname,sep = ' '),
           ...
           count_Sclass = (age >= 21) & ((classS & Class == age_classS) |
                             (!classS & age_group == age_class) |
                               (age >= 55 & Class %in% c("M55AS","M55+AS","M65+AS","M65AS",
                                                         "W55AS","W55+AS","W65+AS","W65AS"))),  #course counts towards S class
           comp_class = (substr(Class,1,1) %in% c("W","M") &
                                age_class >= 16 ) | Class %in% c("MOB","WOB")) %>% #| substr(Class,1,1) == "1"
    filter( !is.na(Place), comp_class) %>% # , comp_class
  ...
  if (i==fix_event) {
    r <- r %>%
      mutate(S_offered = as.numeric(classS)) #ifelse(classS & age_class >= 45, 1, S_offered)
  }
  ...
  r <- r %>%
    ...
    mutate(points=ifelse((!classB | Class %in% c('WOB','MOB')) ,
          ...
          Class= case_when(eventno == MTBO  ~ #& grepl("MTBO", Class)
                           ...
                           Class %in% c("M55AS","M55+AS","M65+AS","M65AS",
                                        "W55AS","W55+AS","W65+AS","W65AS")
                          ~ paste0(sex_prefix, "55+AS"),  #for events with 65+AS
...
summary(results0$YB) #2 missing age group but doesn't matter since they are not eligible
...
results <- results0 %>%
  ...
  group_by(Name) %>%
  mutate(sa_club = max(sa_club, na.rm = T))%>%
  ungroup() %>%
...
table(results$City, useNA="always")
...
r_S <-  results %>% # bind_rows(results, official_S) %>%  #official_classes
  ...
  mutate( sum_S=sum(as.integer(classS),na.rm=T),
          ...
          eligible_s = sum_S == max_S) %>% #ran in AS when offered
...
r_S <- bind_rows(r_S, off_S) %>%
  group_by(Name) %>%
  mutate(eligible_s = all(eligible_s)) %>%
  ungroup()
...
results_short <- r_S %>%
  ...
  filter(!is.na(Class2)) %>%
  select("Class2","Name", event, points, eligible_s) %>% #"Database.Id",
  pivot_wider(id_cols=c("Class2","Name","eligible_s"), #"Database.Id",
              names_from = event,
              values_from = c("points")) %>%
  right_join(total_S) %>%
  group_by(Class2) %>%
...
extra_events <- inner_join(r_A, n_Aclass %>% 
                             select(Class) %>% unique()) %>%
...
results_all <- bind_rows(results_A, 
                         results_short %>% filter(eligible_s) %>% select(!eligible_s), 
                         results_B) %>%
  mutate(Eligible = ifelse(num_events >= min_events, "Y","N"),
         # Eligible = ifelse(type=="AS" & !eligible_s, "N", Eligible)
         )
...
write.csv(results_all %>% select(!type) %>%
            rename(`Number of Events` = num_events), 
          file="../results/2023/Totals 2023.csv", na="", row.names=F)
write.csv(winners_all, file="../results/2023/Winners 2023.csv", na="", row.names=F)
