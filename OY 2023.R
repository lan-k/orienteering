...
##fix Lady Alice Goldfields M55AS

results0 <- results0 %>%
  mutate(Class=ifelse(Class == "M55AS" & eventno == 2, "M65+AS", Class))
...
write.csv(results_all %>% select(!type) %>%
            rename(`Number of Events` = num_events), 
          file="../results/2023/Totals.csv", na="", row.names=F)
write.csv(winners_all, file="../results/2023/Winners.csv", na="", row.names=F)
