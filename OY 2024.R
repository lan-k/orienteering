...
## no need to always enter AS course, must have at least 3 during the year in corect age class 
...
events <- c("SASprintChamps2024.csv"
            ,"ParaWirra2024.csv"
           ,"Woodhouse_Results.csv"
            ,"BroccoliHill2024.csv"
            ,"LegOfMutton2024.csv"
            ,"NightChamps2024 Results.csv"
             ,"MulgaValley2024.csv"
            ,"ParadiseCanyons2024.csv"
            ,"WirreandaFarm_Results.csv"
             ,"MTBO2024.csv"
            ) 

wallaringa_events <- c(3,9)  #8 different format


fix_yr = c(6, wallaringa_events) #  wrong or missing year of births
load("YB_2024.rds")


keep_name = 6 #full names instead of first and last names

S_offered <- c(0,1,0,1,1,1,0,1,0,0)  #short 
...
#officials
off1 <- c("Simon Uppill","Tyson Hillyard",NA_character_)
off2 <- c("David George",NA_character_,NA_character_)  #"Vanessa Round"
off3 <- c("Jessica Jarvis","Peter Ashforth","Tyson Hillyard")
off4 <- c("David Winters","Robert Smith",NA_character_)
off5 <- c("Steve Cooper",NA_character_,NA_character_)
off6 <- c("Robin Uppill",NA_character_,NA_character_)
off7 <- c("Joanna George","David George",NA_character_)
off8 <- c("Robert Boekelaar",NA_character_,NA_character_)
off9 <- c("Jenny Casanova","Steve Cooper","David George")
off10 <- c("Phil Hazell","Nigel Dobson",NA_character_)
...
for (i in seq(length(events))) { 
  ...
  ##find best in Class
  r <- r %>%
    mutate(Class= case_when(# grepl("55AS", Class) |  grepl("65AS", Class) ~ paste0(sex_prefix, "55+AS"),  #NOL events
...
write.csv(results_all %>% select(!type) %>%
            rename(`Number of Events` = num_events), 
          file="../results/2024/Totals.csv", na="", row.names=F)
write.csv(winners_all, file="../results/2024/Winners.csv", na="", row.names=F)
